...
import ProductCard from '../components/makeup/ProductCard';
import CartDrawer from '../components/makeup/CartDrawer';
import { FaEye, FaSmile, FaPaintBrush, FaGem, FaTooth, FaBath, FaWind } from 'react-icons/fa';
import { GiLips, GiRazor } from 'react-icons/gi';
import CategoryBar from '../components/makeup/CategoryBar';
import PromoBanners from '../components/makeup/PromoBanners';
  ...
  useEffect(() => {
    const load = async () => {
      const [catsRes, prodRes] = await Promise.all([
        fetch(`${API_URL}/api/categories?domain=makeup`),
        fetch(`${API_URL}/api/products?domain=makeup`),
      ]);
  ...
  return (
    <div className="main-layout">
      <div className="hero-section text-center py-5 border-bottom">
        <h1 className="fw-bold">Makeup & Skincare</h1>
        <p className="text-muted">Enhance your natural beauty</p>
        <div className="search-box mt-4 mx-auto" style={{ maxWidth: 600 }}>
          <input className="form-control" placeholder="Search makeup..." value={search} onChange={(e) => { const v = e.target.value; setSearch(v); if (!v.trim()) setAppliedSearch(''); }} onKeyDown={(e) => { if (e.key === 'Enter') setAppliedSearch(search); }} />
        ...
        <div className="row">
          <div className="col-lg-2 d-none d-lg-block">
            <div className="sidebar">
              ...
              {categories.map((cat) => {
                const Icon = iconMap[cat.icon] || iconMap[cat.id] || FaSmile;
