...
import { FaBreadSlice, FaBirthdayCake, FaCookie, FaMugHot, FaUtensils } from 'react-icons/fa';
import CategoryBar from '../components/bakery/CategoryBar';
import PromoBanners from '../components/bakery/PromoBanners';
import ProductModal from '../components/ProductModal';
import useScrollToResults from '../hooks/useScrollToResults';
import { includesMatch } from '../utils/search';
import API_URL from '../config';

const BakeryPage = ({ cart, updateCart, removeItem, onCheckout }) => {
  ...
  useEffect(() => {
    const load = async () => {
      const [catsRes, prodRes] = await Promise.all([
        fetch(`${API_URL}/api/categories?domain=bakery`),
        fetch(`${API_URL}/api/products?domain=bakery`),
      ]);
  ...
  return (
    <div className="main-layout">
      <div className="hero-section text-center py-5 border-bottom">
        <h1 className="fw-bold">Bakery & Dairy</h1>
        <p className="text-muted">Freshly baked goodness everyday</p>
        ...
        <div className="row">
          <div className="col-lg-2 d-none d-lg-block">
            <div className="sidebar">
              ...
              {categories.map((cat) => {
                const Icon = iconMap[cat.icon] || iconMap[cat.id] || FaBreadSlice;
