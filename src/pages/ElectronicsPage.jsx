...
import { FaMobileAlt, FaHeadphones, FaTv, FaBlender, FaLaptop } from 'react-icons/fa';
import CategoryBar from '../components/electronics/CategoryBar';
import PromoBanners from '../components/electronics/PromoBanners';
  ...
  useEffect(() => {
    const load = async () => {
      const [catsRes, prodRes] = await Promise.all([
        fetch(`${API_URL}/categories?domain=electronics`),
        fetch(`${API_URL}/products?domain=electronics`),
      ]);
  ...
  return (
    <div className="main-layout">
      <div className="hero-section text-center py-5 border-bottom">
        ...
        <p className="text-muted">Smart tech and essentials</p>
        <div className="search-box mt-4 mx-auto" style={{ maxWidth: 600 }}>
          <input className="form-control" placeholder="Search electronics..." value={search} onChange={(e) => { const v = e.target.value; setSearch(v); if (!v.trim()) setAppliedSearch(''); }} onKeyDown={(e) => { if (e.key === 'Enter') setAppliedSearch(search); }} />
        ...
        <div className="row">
          <div className="col-lg-2 d-none d-lg-block">
            <div className="sidebar">
              ...
              {categories.map((cat) => {
                const Icon = iconMap[cat.icon] || iconMap[cat.id] || FaMobileAlt;
