...
const ProductModal = ({ product, isOpen, onClose, onAdd, onRemove, quantity, cart, onViewRelated, relatedProducts = [] }) => {
  ...
  // Helper to get quantity for related items
        ...
        <div className="pb-modal-scroll-area">
          ...
          <div className="row m-0 border-bottom">
            ...
            <div className="col-lg-6 p-4 p-lg-5">
              ...
              <div className="pb-price mt-4 fs-2 fw-bold text-success">₹{product.price.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                ...
                relatedProducts.map((p) => (
                  <div key={p.id} className="col-6 col-md-4 col-xl-3">
                    <ProductCard 
                      ...
                      onAdd={() => onAdd(p)} 
                      onRemove={() => onRemove(p)}
                      onView={() => onViewRelated(p)} // Switches Modal Content
                    />
                  </div>
                ))
              ) : (
