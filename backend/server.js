...
app.get("/auth/google/callback", async(req, res) => {
    try {
        ...
        const userRes = await fetch(
            "https://www.googleapis.com/oauth2/v2/userinfo", { headers: { Authorization: `Bearer ${tokens.access_token}` } }
        );
        const profile = await userRes.json();
        const jwt = (await
            import ("jsonwebtoken")).default;
        const token = jwt.sign({ id: profile.id, email: profile.email, name: profile.name },
            env.JWT_SECRET || "DEV_SECRET", { expiresIn: "7d" }
        );
