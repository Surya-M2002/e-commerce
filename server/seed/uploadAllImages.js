...
const SRC_IMAGES_DIR = existsSync(NEW_SRC_IMAGES_DIR)
  ? NEW_SRC_IMAGES_DIR
  : OLD_SRC_IMAGES_DIR;
const OUTPUT = path.join(ROOT, "server", "seed", "uploaded.json");
...
const relFromSrc = (abs) =>
  abs.replace(SRC_IMAGES_DIR + path.sep, "").replace(/\\/g, "/");
...
const main = async () => {
  ...
  if (
    ...
    if (args.cloud_name && args.api_key && args.api_secret) {
      ...
      console.error(
        "Missing Cloudinary credentials. Provide CLOUDINARY_URL or pass --cloud_name= --api_key= --api_secret=",
      );
      exit(1);
    }
  }

  const files = walk(SRC_IMAGES_DIR).filter((f) =>
