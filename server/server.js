...
app.get("/", (req, res) => {
  res.send("e-commerce backend is running. Try /ping or /health.");
});
...
// Update current user profile fields (addresses, newsletter, name)
app.put("/users/me", async (req, res) => {
  try {
    ...
    if (Array.isArray(req.body?.addresses))
      update.addresses = req.body.addresses.map(String);
...
// Seller login via mobile OTP
const normalizePhone = (p) =>
  String(p || "")
    .replace(/[^0-9+]/g, "")
    .replace(/^0+/, "");

app.post("/auth/seller/send-otp", async (req, res) => {
  try {
    const phoneRaw = req.body?.phone || "";
    const phone = normalizePhone(phoneRaw);
    const phoneOk = /^\+?\d{10,15}$/.test(phone);
    if (!phoneOk) return res.status(400).json({ error: "Invalid phone" });
    const users = mongoose.connection.collection("users");
    const seller = await users.findOne({ phone, role: "seller" });
    ...
    await users.updateOne(
      { _id: seller._id },
      { $set: { otpHash: hash, otpExpires: expires } },
    );
    const dev = String(env.NODE_ENV || "").toLowerCase() !== "production";
    return res.json({ ok: true, dev_code: dev ? code : undefined });
  } catch {
    return res.status(500).json({ error: "Send OTP failed" });
  }
});

app.post("/auth/seller/verify-otp", async (req, res) => {
  try {
    const phoneRaw = req.body?.phone || "";
    const code = String(req.body?.code || "");
    const phone = normalizePhone(phoneRaw);
    const phoneOk = /^\+?\d{10,15}$/.test(phone);
    if (!phoneOk || !/^\d{6}$/.test(code))
      return res.status(400).json({ error: "Invalid input" });
    const users = mongoose.connection.collection("users");
    const seller = await users.findOne({ phone, role: "seller" });
    if (!seller) return res.status(404).json({ error: "Seller not found" });
    if (
      !seller.otpHash ||
      !seller.otpExpires ||
      Date.now() > Number(seller.otpExpires)
    ) {
      return res.status(400).json({ error: "OTP expired" });
    }
    const bcrypt = (await import("bcryptjs")).default;
    const ok = await bcrypt.compare(code, String(seller.otpHash));
