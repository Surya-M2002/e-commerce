...
const LoginPage = ({ onLoginSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (sellerMode) {
        const phoneNorm = String(phone).replace(/[^0-9+]/g, '').replace(/^0+/, '');
        const phoneOk = /^\+?\d{10,15}$/.test(phoneNorm);
        const otpOk = /^\d{6}$/.test(otp);
        if (!phoneOk || !otpOk) {
          setError(!phoneOk ? "Enter a valid mobile number" : "Enter the 6-digit OTP");
          return;
        }
        const res = await fetch(`${API_URL}/auth/seller/verify-otp`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ phone: phoneNorm, code: otp }),
        });
      ...
      <div className="card shadow border-0 w-100" style={{ maxWidth: 420 }}>
        <div className="card-body p-4 p-md-5">
          ...
          <div className="text-center mb-4">
            ...
            <h4 className="fw-bold mb-1">{sellerMode ? "Seller Login" : "Welcome Back"}</h4>
            <p className="text-muted small">
              {sellerMode ? "Login using your mobile number" : "Login with your email & password"}
            </p>
          </div>

          {/* Social Login */}
          {!sellerMode && (<div className="d-grid gap-2 mb-3">
            <button
                ...
                <div className="mb-2">
                  ...
                  <div className="d-flex gap-2">
                    ...
                    <button
                      ...
                      disabled={loading || !/^\+?\d{10,15}$/.test(String(phone).replace(/[^0-9+]/g, '').replace(/^0+/, ''))}
                      onClick={async () => {
                        setError("");
                        setInfo("");
                        const phoneNorm = String(phone).replace(/[^0-9+]/g, '').replace(/^0+/, '');
                        try {
                          const res = await fetch(`${API_URL}/auth/seller/send-otp`, {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
          ...
          <button
            ...
            disabled={loading}
          >
            {loading ? "Signing in..." : sellerMode ? "Login as Seller" : "Login"}
          </button>
          ...
          <div className="form-check mt-3">
            ...
            <label className="form-check-label" htmlFor="sellerMode">
              Seller login (mobile number & OTP)
            </label>
          </div>
