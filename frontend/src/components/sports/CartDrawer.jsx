      ...
      <div className={`cart-drawer ${isOpen ? "open" : ""}`}>
            ...
            cart.map((item) => (
              <div key={item.id} className="d-flex gap-3 px-4 py-4 border-bottom">
                ...
                <div className="flex-grow-1">
                  ...
                  <div className="text-success fw-bold">₹{item.price.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                  <small className="text-muted">{item.quantity} × {item.unit || "1 pc"}</small>
                </div>
                <div className="text-end">
                  <div className="fw-bold mb-2">₹{(item.price * item.quantity).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
        ...
        {cart.length > 0 && (
          <div className="p-4">
            ...
            >
              <span>Checkout</span>
              <span className="price-pill">₹{total.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
            </button>
          </div>
        )}
      </div>
