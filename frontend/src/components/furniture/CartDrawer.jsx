      ...
      <div className={`cart-drawer ${isOpen ? "open" : ""}`}>
          ...
          {cart.length === 0 ? (
            ...
            cart.map((item) => (
              <div key={item.id} className="d-flex gap-3 px-4 py-4 border-bottom">
                ...
                <img src={normalizeImageUrl(item.img || item.image)} width="60" height="60" className="rounded" alt={item.name} />
                ...
                <div className="flex-grow-1">
                  ...
                  <div className="fw-bold text-success">₹{(item.price * item.quantity).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                </div>
              </div>
            ))
          )}
        ...
        <div className="drawer-footer p-4 border-top">
          <div className="d-flex justify-content-between align-items-center mb-3">
            ...
            <span className="fw-bold text-success">₹{total.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
          </div>
